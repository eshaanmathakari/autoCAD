"""
Pool-specific DXF generator.

Generates pool outline DXFs with proper layers:
  POOL_OUTLINE   - Main pool perimeter
  STAIRS         - Stair treads / risers
  DIMENSIONS     - Dimension entities (with arrows and extension lines)
  EQUIPMENT      - Skimmers, returns, drains
  COPING         - Coping edge
  SAFETY_LEDGE   - Safety/tanning ledge
  LINER_SEAMS    - Seam lines
"""
...
from sketch_to_cad.cad_engine.layers import setup_layers
from sketch_to_cad.cad_engine.stairs import add_stairs
from sketch_to_cad.cad_engine.dimensions import add_pool_dimensions
...
def add_label(msp: Modelspace, text: str, x: float, y: float, height: float = 8) -> None:
    ...
    msp.add_text(
        text,
        dxfattribs={"layer": "POOL_OUTLINE", "height": height},
    ).set_placement((x, y))
...
def generate_pool_dxf(spec: PoolSpec, version: str = "R2010") -> bytes:
    ...
    add_pool_dimensions(msp, doc, spec.edges)
...
def make_l_shaped_pool(
    long_len: float = 300,
    long_width: float = 144,
    short_len: float = 144,
    short_width: float = 96,
...
def make_freeform_pool(name: str = "Freeform Pool") -> PoolSpec:
