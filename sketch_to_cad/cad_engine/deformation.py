...
Takes a reference pool DXF and target dimensions,
then proportionally scales the reference geometry to match.

Uses proper DIMENSION entities instead of TEXT annotations.
"""
...
def _generate_scaled_dxf(edges: list[PoolEdge]) -> bytes:
    """Generate a DXF from scaled edges with proper DIMENSION entities."""
    ...
    # Add proper DIMENSION entities (not TEXT)
    add_pool_dimensions(msp, doc, edges)
